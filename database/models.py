...
class MobileBanner(db.Model):
    ...
    def get_map(self):
        result = {'id': self.id, 'name': self.name, 'picture_url': self.picture_url, 'target': self.target,
                  'banner_type': self.banner_type, 'display_order': self.display_order, 'description': self.description}
        return result
...
class Buyer(db.Model):
    ...
    def get_map(self):
        s = self
        result = {'buyer_id': s.buyer_id, 'qquid': s.qquid, 'account': s.account, 'nick_name': s.nick_name,
                  'real_name': s.real_name,
                  'sex': s.sex, 'email': s.email, 'status': s.status, 'create_time': s.create_time,
                  'create_ip': s.create_ip,
...
class ShopInfo(db.Model):
    ...
    operating_status = db.Column('OperatingStatus', db.String(1))  # 营业状态
    with_draw_account = db.Column('WithDrawAccount', db.String(50))
    has_alipay = db.Column('HasAlipay', db.String(1))
    first_give_coupon = db.Column('FirstGiveCoupon', db.Integer)
    give_limit = db.Column('GiveLimit', db.Integer)
    give_coupon = db.Column('GiveCoupon', db.Integer)
    use_limit = db.Column('UseLimit', db.Integer)
    can_use_coupon = db.Column('CanUseCoupon', db.Integer)
...
def get_map(self):
    ...
    result = {'shop_id': s.shop_id, 'qquid': s.qquid, 'account': s.account, 'shop_type': s.shop_type,
              ...
              'status': s.status,
              'regist_date': s.regist_date, 'regist_ip': s.regist_ip, 'last_login_time': s.last_login_time,
              'login_times': s.login_times,
              'is_validate': s.is_validate, 'shop_property': s.shop_property,
              'is_support_on_line_pay': s.is_support_on_line_pay, 'operating_status': s.operating_status,
              'with_draw_account': self.with_draw_account, 'first_give_coupon': self.first_give_coupon,
              'give_limit': self.give_limit, 'give_coupon': self.give_coupon, 'can_use_coupon': self.can_use_coupon,
...
class Order(db.Model):
    ...
    def get_map(self):
        s = self
        result = {"order_no": s.order_no, 'shop_id': s.shop_id, 'sale_money': str(s.sale_money),
                  'submit_time': s.submit_time, 'send_time': s.send_time,
                  'confirm_time': s.confirm_time, 'freight': str(s.freight), 'address_id': s.address_id,
                  'send_address': s.send_address,
                  'receiver': s.receiver, 'phone': s.phone, 'remark': s.remark, 'status': s.status,
                  'pay_status': s.pay_status, 'update_time': s.update_time, 'get_coupon': s.get_coupon,
                  'use_coupon': s.use_coupon
                  }
        return result
...
class OrderDetail(db.Model):
    ...
    def get_map(self):
        s = self
        result = {'order_no': s.order_no, 'goods_id': s.goods_id, 'batch_no': s.batch_no,
                  'sale_price': str(s.sale_price), 'quantity': s.quantity}
        return result
...
class Comment(db.Model):
    ...
    def get_map(self):
        s = self
        result = {'id': s.id, 'buyer_id': int(s.buyer_id), 'comment_type': s.comment_type, 'order_no': s.order_no,
                  'shop_id': int(s.shop_id),
                  'goods_id': int(s.goods_id), 'level': s.level, 'content': s.content, 'commit_time': s.commit_time,
...
class Message(db.Model):
    ...
    def get_map(self):
        s = self
        result = {'message_id': s.message_id, 'sender_type': s.sender_type, 'sender': s.sender,
                  'sender_name': s.sender_name,
                  'receiver': s.receiver, 'receiver_name': s.receiver_name, 'send_title': s.send_title,
                  'send_content': s.send_content,
                  'reply_time': s.reply_time, 'is_read': s.is_read, 'receiver_type': s.receiver_type,
...
class Photo(db.Model):
    ...
    def get_map(self):
        ...
        result = {'photo_id': s.photo_id, 'photo_type': s.photo_type, 'link_id': s.link_id, 'photo_name': s.photo_name,
                  'photo_path': s.photo_path, 'thumbnail_path': s.thumbnail_path, 'sort_no': s.sort_no,
...
class Purchase(db.Model):
    ...
    def get_map(self):
        result = {'goods_id': self.goods_id, 'batch_no': self.batch_no, 'buy_price': str(self.buy_price),
                  'quantity': self.quantity, 'start_time': self.start_time, 'end_time': self.end_time
                  }
        return result
...
class Activity(db.Model):
    ...
    def get_map(self):
        s = self
        result = {'id': s.id, 'type': s.type, 'shop_id': s.shop_id, 'title': s.title, 'content': s.content,
                  'count': s.count, 'sort_no': s.sort_no, 'is_top': s.is_top, 'is_hot': s.is_hot,
                  'publisher': s.publisher, 'publish_time': s.publish_time, 'updater': s.updater,
                  'update_time': s.update_time, 'seo_title': s.seo_title, 'seo_content': s.seo_content,
                  'del_flag': s.del_flag, 'call_index': s.call_index}
        return result
